...
def time_of_travel(lake_file, upstream_file, sam_output_file, output_file, convolve=True, aggregate=False):
    ...
    volume_dict, outflow_dict, outlet_dict, residence_times, wb_to_reach, reach_to_wb = read.unpickle(lake_file)
    ...
    lentic_reaches, lotic_reaches, upstream_lentics = \
        functions.classify_reaches(sam_lookup.keys(), outlet_dict, wb_to_reach)
    ...
    for i, reach in enumerate(lotic_reaches):
        ...
        if reach_address:  # Make sure that the reach is in the path map
            ...
            if upstream_paths.any():  # Check that reach was found in the path matrix.
                ...
                local_output = \
                    functions.process_reservoirs(local_output, local_lookup, reach_to_wb, upstream_paths,
                                                  residence_times)
