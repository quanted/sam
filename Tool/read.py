...
def flows(flow_file, dates, id_field="COMID", filter=None):
    ...
    data = data.as_matrix()
    months = np.array([date.month for date in dates]) - 1
    for row in data:
        recipe_id = int(row[0])
        length = row[1]
        q = row[2:14]  # Q values start at column 2
        v = row[15:27]  # V values start at column 2
        flow = ParameterSet({'q': q[months], 'v': v[months], 'l': length})
...
def hydro(hydro_path, reach, start_count, years, process_erosion=False):
    ...
    if os.path.isfile(hydro_file):  # MMF - with erosion, hydro files will now include 4 additional columns for erosion 2010-2014
        ...
        # JCH - fix this in hydro file eventually
        header = [runoff_header.format(y) for y in years]
        if process_erosion:
            header += [erosion_header.format(y) for y in years]
        data = pd.read_csv(hydro_file, skiprows=1, sep='\s+', dtype=np.float, names=header)

        # Check for all required field headings
        missing_fields = set(header) - set(data.columns.values)
        if any(missing_fields):
            print("Missing the following fields in hydro file {}: \n\t{}".format(
                hydro_file, ", ".join(map(str, missing_fields))))
            return

        for year in years:
            yield year, data[data.index >= start_count][["R{}_m3".format(year), "E{}_kg".format(year)]].as_matrix().T
