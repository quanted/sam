...
#   Addition of gamma1 and gamma2 degradation rates
    ...
    if (gamma1 <= 1e-18): gamma1 = 1e-18
    if (gamma2 <= 1e-18): gamma2 = 1e-18
   ...
   #MMF - reset starting values
    ...
    mn1=0
    mn2=0
    DELT = 86400.  #seconds, time step ONE DAY
    ...
    for d in range(1, daily_concentration.size):
        ...
        new_aqconc1, new_aqconc2, aqconc_avg1, aqconc_avg2 = simuldiff2(A,B,E,F,aqconc1,aqconc2,DELT)
...
#MMF  Calculates Solute Holding capacities and mass transfer between water column and benthic regions 
def soluteholdingcap(koc):
   ...
   # solute holding capacity in region 2
    ...
    return capacity1, capacity2, fw1, fw2, theta, sed_conv_factor, omega
   ...
   #  mn2 IS OUTPUT VALUE FOR m2 AFTER TIME T
   #  mavg1 IS AVERAGE VALUE OF m1 OVER TIME T
   def simuldiff2(A,B,E,F,m1,m2,Tend):
    af=A+F
    fxa=F*A
    bxe=B*E
    dif=4*(fxa-bxe)
    ...
    mavg1=(term1+term2+term3+term4)/Tend     #mavg1=(term1+term2+term3+term4)/(T2-T1)
    ...
    mavg2=(term1*DD+term2*EE+term3*DD+term4*EE)/Tend  #average compartment 2 concentration
