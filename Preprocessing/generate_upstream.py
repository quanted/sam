import os
import numpy as np
from trip_tools import get_nhd, read_dbf
...
def generate_navigator(region, output_directory,
                       travel_time=False, filename="upstream", overwrite=True):
    ...
    def get_gridcode(gridcode_table):
        input_array = np.array(read_dbf(gridcode_table, ["GRIDCODE", "FEATUREID"])).T
        out_array = np.zeros(input_array[0].max() + 1)
        out_array[input_array[0]] = input_array[1]
        return out_array
    ...
    def write_outfile(outfile, paths, path_map, conversion_array, attributes, names, gridcode_array):
        attribute_dict = {name: attributes[i] for i, name in enumerate(names)}
        np.savez_compressed(outfile, paths=paths, path_map=path_map, alias_index=conversion_array,
                            gridcode_array=gridcode_array, **attribute_dict)
    ...
    def fetch_times():
        ...
        for comid in set(velocity_dict.keys()) & set(length_dict.keys()):
            velocity = velocity_dict.get(comid, 0.0) * 0.3048  # ft/s -> m/s
            length = length_dict.get(comid, 0.0) * 1000.  # km -> m
            if velocity and length:
                time_dict[comid] = (length / velocity) / 86400.  # m/s -> m/d
            else:
                time_dict[comid] = 0.0
        return time_dict
    ...
    if not os.path.exists(outfile) or overwrite:
        ...
        gridcode_array = get_gridcode(gridcode_table)

        write_outfile(outfile, paths, path_map, conversion_array, attribute_matrix, names, gridcode_array)
...
def main():
    # Output path
    output_folder = r"S:\bin\Preprocessed\Upstream"
    travel_time = True
